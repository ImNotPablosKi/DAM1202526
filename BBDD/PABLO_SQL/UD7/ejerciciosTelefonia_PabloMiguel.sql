-- Ejercicios Consultas BBDD Telefonía
-- Pablo Miguel, DAM1

-- 1:

SELECT nombre, apellidos FROM clientes WHERE padrino IN (SELECT padrino FROM clientes WHERE edad > 18)
	AND tarifa IN (SELECT id_tarifa FROM tarifas WHERE nombre LIKE 'Barata');
...
SELECT c.nombre, c.apellidos, f.nombre, f.establecimiento, COUNT(l.id_llamada) FROM llamadas l INNER JOIN clientes c ON l.id_cliente=c.id_cliente
...
-- 5:
SELECT DISTINCT l.* FROM llamadas l INNER JOIN clientes c ON l.id_cliente=c.id_cliente 
...
-- 6:
SELECT c.nombre, c.apellidos FROM clientes c INNER JOIN tarifas t ON c.tarifa=t.id_tarifa 
WHERE (t.nombre LIKE 'Normal' OR t.nombre LIKE 'Buena')	AND NOT EXISTS (SELECT 1 FROM llamadas WHERE id_cliente=c.id_cliente AND tipo LIKE 'N');

-- 7:
SELECT c.*, COUNT(l.id_llamada) FROM llamadas l INNER JOIN clientes c ON l.id_cliente=c.id_cliente 
